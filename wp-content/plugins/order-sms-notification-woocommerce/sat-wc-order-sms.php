...
class Sat_WC_Order_SMS {
    ...
    public function __construct() {
        ...
        add_action( 'admin_notices', array( $this, 'show_pro_nitices' ) );
    ...
    public function plugin_row_meta_link( $plugin_meta, $plugin_file, $plugin_data, $status ) {
        ...
        if( isset( $plugin_data['slug'] ) && ( $plugin_data['slug'] == 'order-sms-notification-woocommerce' ) && ! defined( 'SATSMS_DIR' ) ) {
           $plugin_meta[] = sprintf( '<a href="%s" target="_blank">%s</a>',
                esc_url( 'http://codecanyon.net/item/woocommerce-order-sms-notification/8339735/' ),
                __( 'Get Premium', 'satosms' )
            );
        }
    ...
    function show_pro_nitices() {
        $screen = get_current_screen();

        if( $screen->parent_base == 'sat-order-sms-notification-settings' ) {
            ?>
                <div class="updated notice notice-success is-dismissible">
                    <p><?php _e( 'In premium version there are lots of extended features. To get the pro version of this plugin checkout here <a target="_blank" href="http://codecanyon.net/item/woocommerce-order-sms-notification/8339735">Order SMS Notification - WooCommerce ( Pro )</a>', 'satosms' ); ?></p>
    ...
    public  function trigger_after_order_place( $order_id, $old_status, $new_status ) {
        ...
        if( in_array( $new_status, $order_status_settings ) ) {
            ...
            if( $want_to_notify_buyer ) {
                ...
                $buyer_sms_data['sms_body'] = $this->pharse_sms_body( $buyer_sms_body, $new_status, $order_id );
